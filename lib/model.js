...
Model.deleteOne = function deleteOne(filter, callback) {
  ...
  if (callback == undefined) {
    return new Promise(function (resolve, reject) {
      query.mainDelete(tableName, Connection, filter, true, function (err) {
        if (err) reject(err);
      });
    });
  } else {
    query.mainDelete(tableName, Connection, filter, true, callback);
  }
};
...
Model.deleteMany = function deleteMany(filter, callback) {
  ...
  if (callback == undefined) {
    return new Promise(function (resolve, reject) {
      query.mainDelete(tableName, Connection, filter, function (err) {
        if (err) reject(err);
      });
    });
  } else {
